...
@echo off
chcp 65001 >nul
title 3080 Dual-Card qwen38-27b NO-VISION 256K Launch
setlocal
...
wsl -d Ubuntu -u root -- systemctl start sglang-qwen38-novision
if errorlevel 1 (
  echo [Error] systemctl start failed. Check: wsl -d Ubuntu -u root -- journalctl -u sglang-qwen38-novision -n 50
  goto end
)
...
echo   wsl -d Ubuntu -u root -- journalctl -u sglang-qwen38-novision -n 50
goto end
